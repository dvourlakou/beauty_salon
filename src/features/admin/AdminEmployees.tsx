            ...
            {showNewEmployee && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
                    ...
                    <div className="flex gap-2 mt-3">
                        ...
                        <button onClick={() => setShowNewEmployee(false)} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">Ακύρωση</button>
                    </div>
                </div>
            )}

            <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y divide-gray-50">{employees.map(employee) => (
                    <div key={employee.id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50">
                {editingEmployee?.id !== employee.id ? (
                      <>
                       <div>
                         <span  className="font-medium text-gary-700">{employee.name}</span>
                         <span className="ml-3 text-sm text-gray-400">{specializationLabels[employee.specialization]}</span>
                       </div>
                       <div className="flex gap-2">
                         <button onClick={() => setEditingEmployee(employee)} className="text-gray-400 hover:text-blue-500"><Edit size={18} /></button>
                         <button onClick={() => handleDelete(employee.id)} className="text-gray--400 hover:text-red-500"><Trash2 size={18} /></button>
                       </div>
                       </>
                ) : (
                       <div className="flex  items-center gap-3 w-full">
                          ...
                          <button onClick={() => handleUpdate(editingEmployee)} className="text-green-500 hover:text-green-700"><Save size={18} /></button>
                          <button onClick={() => setEditingEmployee(null)} className="text-gray-400 hover:text-gray-500"><X size={18} /></button>
                      </div>
                    )}
            </div>
            ))}
