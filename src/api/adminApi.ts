...
export const adminApi = {
  ...
  getWeeklyAppointments: async (): Promise<Appointment[]> => {
    const response = await axiosInstance.get<Appointment[]>('/admin/appointments');
    return response.data;
  },

  getEmployeeWorkload: async (): Promise<EmployeeWorkload[]> => {
    const response = await axiosInstance.get<EmployeeWorkload[]>('/admin/workload');
    return response.data;
  },
  ...
  updateAppointmentStatus: async (id: number, status: string): Promise<Appointment> => {
    const response = await axiosInstance.patch<Appointment>(`/admin/appointments/${id}/status`, { status });
    return response.data;
  },
  ...
  updateEmployee: async (id: number, data: Partial<Employee>): Promise<Employee> => {
    const response = await axiosInstance.put<Employee>(`/admin/services/${id}`, data);
    return response.data;
  },
